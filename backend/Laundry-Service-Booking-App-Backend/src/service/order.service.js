...
import { createNotification } from "../controller/admin.controller.js";
import { getDeliveryCharge } from "./payment.service.js";
...
export const CreateOrderService = async (req) => {
  try {
    ...
    const { items, address, notes, couponCode, paymentMethod: rawPaymentMethod, latitude, longitude, billingInfo, shippingInfo, schedule, deliveryType, deliverySpeedCharge, deliveryCharge: clientDeliveryCharge, subtotal: clientSubtotal, discount: clientDiscount, totalPayment: clientTotalPayment } = req.body;
    // Normalize cashOnDelivery / cod aliases to "cash"
    const paymentMethod = (rawPaymentMethod === "cashOnDelivery" || rawPaymentMethod === "cod") ? "cash" : (rawPaymentMethod || "cash");
    ...
    if (couponCode) {
      ...
      if (coupon && subtotal >= coupon.minOrderValue && (!coupon.usageLimit || coupon.usedCount < coupon.usageLimit)) {
        // Check per-user limit
        if (coupon.perUserLimit && userId) {
          const userUsage = coupon.usedBy?.find(u => u.userId?.toString() === userId);
          if (userUsage && userUsage.count >= coupon.perUserLimit) {
            // User exceeded their per-user limit, skip discount
          } else {
            discount = coupon.discountType === "percentage"
              ? Math.min(subtotal * coupon.discountValue / 100, coupon.maxDiscount || Infinity)
              : coupon.discountValue;
            coupon.usedCount += 1;
            const existingUser = coupon.usedBy?.find(u => u.userId?.toString() === userId);
            if (existingUser) { existingUser.count += 1; }
            else { coupon.usedBy = [...(coupon.usedBy || []), { userId, count: 1 }]; }
            await coupon.save();
          }
        } else {
          discount = coupon.discountType === "percentage"
    ...
    // Compute delivery charge server-side to prevent client-side tampering
    const baseDeliveryCharge = await getDeliveryCharge(deliveryType || "standard");
    const totalPayment = Math.max(subtotal - discount + baseDeliveryCharge + speedCharge, 0);
    ...
    const order = await Order.create({
      ...
      // Cash/COD: pending until delivery collects payment.
      // PayPal/Stripe: capture happens before order creation, so mark as paid immediately.
    ...
    createNotification({
      ...
      message: `Order #${order._id.toString().slice(-6).toUpperCase()} placed - ${order.itemsSummary} - Total: ${order.totalPayment}`,
...
export const AdminGetOrderByIdService = async (req) => {
  try {
    const { id } = req.params;
    // Try to find by MongoDB _id first, then by orderId if that fails
    ...
    // If not found by _id, try searching by orderId field
    if (!order) {
...
export const AdminGetNearbyDeliveryBoysService = async (req) => {
  try {
    ...
    // Admin page needs ALL delivery boys (available + offline) for management
    let filter = { role: "delivery", isBlocked: { $ne: true } };
    ...
    const deliveryBoys = await User.find(filter).select("name phone email profileImage isAvailable totalEarnings pendingEarnings");
