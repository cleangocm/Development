...
import { EncodeToken, GenerateRefreshToken, HashRefreshToken } from "../utils/TokenHelper.js";
...
const REFRESH_TOKEN_TTL_MS = parseInt(process.env.REFRESH_TOKEN_TTL_DAYS || "30", 10) * 24 * 60 * 60 * 1000;

/**
 * Issue an access token + refresh token pair and persist the hashed refresh
 * token to the user document (keeping at most 5 active devices).
 */
const issueTokenPair = async (user) => {
  const accessToken = EncodeToken(user.email, user._id.toString());
  const refreshToken = GenerateRefreshToken();
  const tokenHash = HashRefreshToken(refreshToken);
  const expiresAt = new Date(Date.now() + REFRESH_TOKEN_TTL_MS);

  // Prune expired tokens then append new one (max 5 concurrent sessions)
  const active = (user.refreshTokens || []).filter((t) => t.expiresAt > new Date());
  if (active.length >= 5) active.shift(); // drop oldest
  active.push({ tokenHash, expiresAt });
  user.refreshTokens = active;
  await user.save();
...
/**
 * Refresh Token — exchange a valid refresh token for a new access + refresh token pair.
 * The old refresh token is rotated (deleted) on every use.
 */
export const RefreshTokenService = async (req) => {
  try {
    ...
    if (!refreshToken) {
      return { status: "failed", message: "Refresh token is required" };
    }

    const tokenHash = HashRefreshToken(refreshToken);

    // Find the user who owns this hashed refresh token and whose token is not expired
    const user = await User.findOne({
