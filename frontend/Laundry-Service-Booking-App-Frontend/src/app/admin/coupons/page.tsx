...
const AdminCouponsPage = () => {
  ...
  const [formPerUserLimit, setFormPerUserLimit] = useState('');
  ...
  const fetchCoupons = useCallback(async () => {
    try {
      ...
      // Backend returns { data: { coupons, total, page, totalPages } }
  ...
  const openEditModal = (coupon: Coupon) => {
    ...
    setFormPerUserLimit(coupon.perUserLimit ? String(coupon.perUserLimit) : '');
  ...
  const handleSaveCoupon = async () => {
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        perUserLimit: formPerUserLimit ? Number(formPerUserLimit) : null,
  ...
  // Filter coupons - ensure coupons is always an array
  ...
  const totalUsed = Array.isArray(coupons) ? coupons.reduce((acc, c) => acc + (c.usedCount ?? 0), 0) : 0;
  const activeCoupons = Array.isArray(coupons) ? coupons.filter(c => c.isActive && !isExpired(c)) : [];
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{Array.isArray(coupons) ? coupons.length : 0}</p>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{Array.isArray(coupons) ? coupons.filter(c => isExpired(c)).length : 0}</p>
                ...
                return (
                  ...
                  >
                    <div className="p-6">
                      ...
                      {/* Discount Display */}
                      <div className="flex items-center gap-4 mb-4 p-4 bg-linear-to-r from-[#00BFA6]/10 to-[#0F2744]/10 dark:from-[#00BFA6]/20 dark:to-[#0F2744]/20 rounded-xl">
                        <div className="flex items-center gap-2">
                          {coupon.discountType === 'percentage' ? (
                            <>
                              <FiPercent className="w-8 h-8 text-[#00BFA6]" />
                              <span className="text-3xl font-bold text-[#00BFA6]">{coupon.discountValue}%</span>
                            </>
                          ) : (
                            <>
                              <FiDollarSign className="w-8 h-8 text-[#00BFA6]" />
                              <span className="text-3xl font-bold text-[#00BFA6]">{coupon.discountValue}</span>
                            </>
                          )}
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">
                          {coupon.minOrderValue > 0 && <p>Min. Order: {formatPrice(coupon.minOrderValue)}</p>}
                          {coupon.maxDiscount && <p>Max. Discount: {formatPrice(coupon.maxDiscount)}</p>}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col gap-1 text-sm text-gray-500 dark:text-gray-400 mb-4">
                        ...
                        {coupon.perUserLimit && (
                          <div className="flex items-center gap-2">
                            ...
                            <span>Per user: {coupon.perUserLimit === 1 ? 'Single use' : `${coupon.perUserLimit} uses max`}</span>
                          </div>
                        )}
                      </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input 
                    type="text" 
                    value={formCode}
                    onChange={(e) => setFormCode(filterSafeText(e.target.value))}
                ...
                <div>
                  ...
                  <input 
                    type="text" 
                    value={formTitle}
                    onChange={(e) => setFormTitle(filterSafeText(e.target.value))}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setFormDiscountValue(filterNumberInput(e.target.value))}
                      placeholder="0"
                      className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white" 
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setFormMinOrderValue(filterNumberInput(e.target.value))}
                      placeholder="0"
                      className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white" 
                    />
                  </div>
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setFormMaxDiscount(filterNumberInput(e.target.value))}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setFormUsageLimit(filterDigitsOnly(e.target.value))}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Per-User Usage Limit</label>
                  <input 
                    type="number" 
                    value={formPerUserLimit}
                    onChange={(e) => setFormPerUserLimit(filterDigitsOnly(e.target.value))}
                    placeholder="Unlimited (e.g. 1 = one-time use per user)"
                    className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white" 
                  />
                  <p className="text-xs text-gray-400 mt-1">Set 1 for single-use per user. Leave empty for unlimited.</p>
                </div>
