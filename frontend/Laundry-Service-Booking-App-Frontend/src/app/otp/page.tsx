...
const OtpPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-gray-50 dark:bg-gray-900">
      ...
      <div className="hidden lg:flex lg:w-1/2 items-center justify-center p-8 xl:p-16 bg-white dark:bg-gray-800">
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-6 md:p-8 lg:p-12 bg-white dark:bg-gray-900 lg:bg-gray-50 lg:dark:bg-gray-900 min-h-screen lg:min-h-0">
        <div className="w-full max-w-95 sm:max-w-105 bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-2xl lg:shadow-xl animate-fade-in-up">
          {/* Logo */}
          ...
          {/* Dev Mode OTP Banner */}
          {devOtp && (
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-300 dark:border-yellow-700 px-4 py-3 rounded-lg text-sm animate-fade-in mb-4">
              <p className="font-semibold text-yellow-800 dark:text-yellow-200">🔧 Dev Mode — Email/SMS unavailable</p>
              <p className="text-yellow-700 dark:text-yellow-300 mt-1 mb-2">
                Your OTP: <span className="font-mono font-bold text-lg tracking-widest text-yellow-900 dark:text-yellow-100">{devOtp}</span>
              </p>
              <button
                type="button"
                onClick={() => {
          ...
          <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
              ...
              <div className="flex justify-center gap-2 sm:gap-3" onPaste={handlePaste}>
                {otp.map((digit, index) => (
                  <input
                    ...
                    className={`w-10 h-12 sm:w-12 sm:h-14 md:w-14 md:h-16 text-center text-lg sm:text-xl md:text-2xl font-bold border-2 rounded-lg sm:rounded-xl text-[#0f2744] dark:text-white bg-white dark:bg-gray-700 focus:outline-none transition-all duration-300 ${
                      (error || validationError)
                        ? 'border-red-500 focus:border-red-500 animate-shake'
                        : digit
                        ? 'border-[#00BFA6] bg-[#00BFA6]/5'
                        : 'border-gray-200 focus:border-[#00BFA6]'
                    }`}
                  />
                ))}
              </div>
            ...
            <div className="text-center">
              <p className="text-xs sm:text-sm text-[#5a6a7a] dark:text-gray-400">
                {canResend ? (
                  <button
                    type="button"
