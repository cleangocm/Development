...
const SuccessPage = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 p-4 sm:p-6">
      <div className={`w-full max-w-80 sm:max-w-95 bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-2xl shadow-xl text-center transition-all duration-500 ${mounted ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
        {/* Success Icon */}
        ...
        <div className={`transition-all duration-500 delay-[400ms] ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-[#0f2744] dark:text-white mb-2">
        ...
        <Link
          href="/login"
          className={`block w-full bg-[#00BFA6] text-white py-3 sm:py-3.5 rounded-lg sm:rounded-xl font-semibold text-sm sm:text-base transition-all duration-500 delay-[500ms] hover:bg-[#00A892] hover:shadow-lg mt-6 sm:mt-8 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
        >
          Login Now
        </Link>
      </div>
