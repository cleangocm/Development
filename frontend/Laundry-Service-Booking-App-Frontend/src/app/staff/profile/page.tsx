...
const StaffProfilePage = () => {
  ...
  const handleCancel = () => {
    // Reset form state to original auth store values
    setName(user?.name || '');
    setPhone(user?.phone || '');
    setIsEditing(false);
  };
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
          ...
          <div className="space-y-4">
            <div>
              ...
              {isEditing ? (
                <input type="text" value={name} onChange={(e) => setName(filterNameInput(e.target.value))}
            ...
            <div>
              ...
              {isEditing ? (
                <input type="text" value={phone} onChange={(e) => setPhone(filterPhoneInput(e.target.value))}
                  className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 text-sm" />
              ) : (
          ...
          <div className="mt-6 flex gap-3">
            {isEditing ? (
              <>
                ...
                <button onClick={handleCancel} className="px-6 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
                  Cancel
                </button>
              </>
            ) : (
