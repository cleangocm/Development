...
import { filterDigitsOnly, filterNameInput } from '@/lib/inputValidation';
...
const PaymentMethodPage = () => {
  ...
  const detectCardType = (number: string): { type: PaymentCard['type']; name: string } => {
    const cleaned = number.replace(/\s/g, '');
    if (/^4/.test(cleaned)) return { type: 'visa', name: 'Visa' };
    if (/^5[1-5]/.test(cleaned) || /^2[2-7]/.test(cleaned)) return { type: 'mastercard', name: 'Master Card' };
  ...
  const handleAddCard = () => {
    ...
    if (!cleaned || cleaned.length < 13 || cleaned.length > 19 || !newCard.cardHolder.trim()) return;
    if (newCard.expiry.length < 4) return;
    if (newCard.cvv.length < 3) return;

    const lastFour = cleaned.slice(-4);
    const { type, name } = detectCardType(cleaned);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        <div className="p-4 sm:p-6 space-y-4">
          {cards.map((card) => (
            <div
              key={card.id}
              className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-600 rounded-xl hover:border-[#00BFA6]/30 transition-colors group"
            >
              ...
              <button
                ...
                className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors sm:opacity-0 sm:group-hover:opacity-100"
              >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 p-4 bg-[#0F2744] dark:bg-[#00BFA6] text-white rounded-xl font-medium hover:bg-[#1a3a5c] dark:hover:bg-[#00A892] transition-colors"
          >
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-2xl">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewCard({ ...newCard, cardNumber: filterDigitsOnly(e.target.value) })}
                    placeholder="1234 5678 9012 3456"
                    maxLength={19}
                    className="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#00BFA6] focus:border-transparent outline-none bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewCard({ ...newCard, cardHolder: filterNameInput(e.target.value) })}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={newCard.cvv}
                      onChange={(e) => setNewCard({ ...newCard, cvv: filterDigitsOnly(e.target.value) })}
                      placeholder="123"
                      maxLength={4}
                      className="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#00BFA6] focus:border-transparent outline-none bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                    />
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  ...
                  className="flex-1 px-6 py-2.5 bg-[#0F2744] dark:bg-[#00BFA6] text-white rounded-lg font-medium hover:bg-[#1a3a5c] dark:hover:bg-[#00A892] transition-colors"
                >
                  Add Card
                </button>
              </div>
