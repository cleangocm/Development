...
const DeliveryProfilePage = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'name') filteredValue = filterNameInput(value);
    else if (name === 'phone') filteredValue = filterPhoneInput(value);
    else if (name === 'address') filteredValue = filterSafeText(value);
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center gap-4">
          <div className="relative">
            <div className="w-20 h-20 rounded-full bg-[#0e6251] flex items-center justify-center text-white text-2xl font-bold overflow-hidden">
              {avatarPreview ? (
            ...
            <label className="absolute -bottom-1 -right-1 w-7 h-7 bg-white dark:bg-gray-700 rounded-full shadow flex items-center justify-center border border-gray-200 dark:border-gray-600 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors">
              <FiCamera className="w-3.5 h-3.5 text-gray-500 dark:text-gray-400" />
              <input
                type="file"
                accept="image/*"
                className="hidden"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input name="email" value={formData.email} onChange={handleChange} disabled
                className="w-full pl-10 pr-4 py-3 bg-gray-100 dark:bg-gray-600 border border-gray-200 dark:border-gray-600 rounded-xl text-sm text-gray-500 dark:text-gray-400 cursor-not-allowed" />
            </div>
          </div>
