...
import { FiMenu, FiBell, FiUser, FiLogOut, FiSun, FiMoon, FiX } from 'react-icons/fi';
...
const DeliveryHeader = ({ onMenuClick }: DeliveryHeaderProps) => {
  ...
  useEffect(() => {
    ...
    if (!notifFetched.current) { notifFetched.current = true; doFetch(); }
      ...
      <div className="flex items-center justify-between px-4 py-3 lg:px-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`relative w-14 h-7 rounded-full transition-all duration-500 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#148f77]/50 ${
              isDark
          ...
          <div className="relative">
            <button onClick={() => setShowNotifications(!showNotifications)} className="relative p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
              ...
              <>
                ...
                <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 z-50 overflow-hidden">
                  <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                    ...
                    <button onClick={() => setShowNotifications(false)} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"><FiX className="w-4 h-4 text-gray-400" /></button>
                  </div>
                  <div className="max-h-80 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700">
                    {notifications.length === 0 ? (
                      ...
                      <div key={n._id} className={`p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer ${!n.isRead ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}
                        onClick={() => { if (!n.isRead) markAsRead(n._id); if (n.orderId) { setShowNotifications(false); router.push(`/delivery/pickup`); } }}>
            ...
            <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="flex items-center gap-2 p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
              {user?.profileImage ? (
                <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-200">
                  <SafeImage src={user.profileImage} alt={user.name || 'Delivery'} variant="avatar" width={32} height={32} className="w-full h-full object-cover" />
                </div>
              ) : (
            ...
            {showProfileMenu && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 z-50 overflow-hidden">
                  <div className="py-2">
                    <Link href="/delivery/profile" onClick={() => setShowProfileMenu(false)} className="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
