...
import { FiMenu, FiBell, FiSearch, FiUser, FiLogOut, FiSun, FiMoon, FiX } from 'react-icons/fi';
      ...
      <div className="flex items-center justify-between px-4 py-3 lg:px-6">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`relative w-14 h-7 rounded-full transition-all duration-500 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#1a5276]/50 ${
              isDark
                  ...
                  <div className="max-h-80 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700">
                    {notifications.length === 0 ? (
                      ...
                      <div key={n._id} className={`p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer ${!n.isRead ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}
                        onClick={() => { if (!n.isRead) markAsRead(n._id); if (n.orderId) { setShowNotifications(false); router.push(`/staff/orders`); } }}>
            ...
            >
              {user?.profileImage ? (
                <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-200">
                  <SafeImage src={user.profileImage} alt={user.name || 'Staff'} variant="avatar" width={32} height={32} className="w-full h-full object-cover" />
                </div>
              ) : (
